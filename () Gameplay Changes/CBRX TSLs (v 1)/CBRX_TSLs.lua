...
cbrxTSLs["CIVILIZATION_TOSA"] = {X=99, Y=62, O="T"}				--Shikoku
...
local observerTechs = {2, 4, 5, 6, 9, 11}

function addHarbors(iPlayer)
	local player = Players[iPlayer]
	if not player then return end
	if player:GetNumCities() > 3 then return end
	...
	if cbrxTSLs[sCivilizationType].O == "T" then
		for city in player:Cities() do
			if city:IsCoastal() then city:SetNumRealBuilding(23, 1) end
		end
	end
