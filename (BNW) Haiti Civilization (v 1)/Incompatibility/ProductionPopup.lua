...
function AddProductionButton( id, description, orderType, turnsLeft, column, isDisabled, ePurchaseYield )	
	...
	if column == 1 then -- we are a unit
		...
		local thisUnitID = GameInfo.Units[id].ID;

-- Changed for Missionary and Inquisitor overrides -------------------------------------------
	
	local iMissionaryID = GameInfo.Units.UNIT_MISSIONARY.ID
	local iMissionaryClass = GameInfo.UnitClasses.UNITCLASS_MISSIONARY.ID

	local iInquisitorID = GameInfo.Units.UNIT_INQUISITOR.ID
	local iInquisitorClass = GameInfo.UnitClasses.UNITCLASS_INQUISITOR.ID

	if thisUnitID == iMissionaryID then
		...
		for row in GameInfo.Civilization_UnitClassOverrides() do
			...
			if iCiv == CivType then
				...
				if iClass == iMissionaryClass then
					thisUnitInfo = GameInfo.Units[row.UnitType]
					iUnit = GameInfo.Units[row.UnitType].ID
					thisUnitID = iUnitType;

					for urow in GameInfo.Units() do
						iUnitU = GameInfo.Units[urow.Type].ID
						if iUnit == iUnitU then
	...
	if thisUnitID == iInquisitorID then
		local CivType = Game.GetActiveCivilizationType()
		for row in GameInfo.Civilization_UnitClassOverrides() do
			iCiv = GameInfo.Civilizations[row.CivilizationType].ID
			if iCiv == CivType then
				iClass = GameInfo.UnitClasses[row.UnitClassType].ID
				if iClass == iInquisitorClass then
					thisUnitInfo = GameInfo.Units[row.UnitType]
					iUnit = GameInfo.Units[row.UnitType].ID
					thisUnitID = iUnitType;
...
-- Changed for Missionary and Inquisitor overrides /end -------------------------------------------


		-- Portrait
		local portraitOffset, portraitAtlas = UI.GetUnitPortraitIcon(iUnit, pCity:GetOwner());
