...
function cbr_save( target, key, value, mod, cacheState )
  local t = type( target );
  if not t == "table" then
  ...
  pSave = cbr_load( target, nil, false, iCache );
...
function cbr_load( target, key, mod, cacheState )
  local t = type( target );
  if not t == "table" then
