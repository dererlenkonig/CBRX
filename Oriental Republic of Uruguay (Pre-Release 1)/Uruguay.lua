...
function GoldBuyDown(iPlayer, iParty, iGoldThreshold)
	...
	for pCity in pPlayer:Cities() do
		...
		if pPlayer:GetCurrentEra() < 2 and GetShouldPurchaseSettlers(iPlayer) then
			iThreshold = 6;
		end
		...
		if iRand < iThreshold then
			...
			for i = 0, iUnitsSize do
				...
				if pUnit ~= nil and pCity:IsCanPurchase(true, true, iUnit, -1, -1, YieldTypes.YIELD_GOLD) then
					if (iUnit == GameInfoTypes.UNIT_SETTLER and pPlayer:GetCurrentEra() < 3 and GetShouldPurchaseSettlers(iPlayer)) or iUnit == iCascosUnit then
						--print(iUnit .. " valid * 5!");
						validUnits[i4] = pUnit;
						i4 = i4 + 1;
						validUnits[i4] = pUnit;
