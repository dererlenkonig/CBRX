print("This is the modded UnitPanel from 'UI - Condensed Promotions'")
...
function UpdateUnitActions( unit )
	...
	Controls.UnitStatStrength:SetHide(false);
	Controls.UnitStatNameStrength:SetHide(false);
    ...
    local function SetToolTip(sTitle, sToolTip)
        ...
        ttTable.UnitActionHelp:SetText(string.format("[NEWLINE]%s", Locale.ConvertTextKey(sToolTip)));
    ...
    for _, action in pairs(addinActions) do
        if bOkayToProcess and (action.Condition == nil or
        (type(action.Condition) == "function" and action.Condition(action, unit)) or
        (type(action.Condition) ~= "function" and action.Condition))
        then
            local instance;
            ...
            if ((type(action.Disabled) == "function" and action.Disabled(action, unit)) or
                (type(action.Disabled) ~= "function" and action.Disabled))
            then
                instance.UnitActionButton:SetAlpha(0.4);
                instance.UnitActionButton:SetDisabled(true);
            else
    ...
    for _, build in pairs(addinBuilds) do
            if bOkayToProcess and (build.Condition == nil or
        (type(build.Condition) == "function" and build.Condition(build, unit)) or
        (type(build.Condition) ~= "function" and build.Condition))
        then
            ...
            if ((type(build.Disabled) == "function" and build.Disabled(build, unit)) or
                (type(build.Disabled) ~= "function" and build.Disabled))
            then
                instance.UnitActionButton:SetAlpha(0.4);
                instance.UnitActionButton:SetDisabled(true);
            else
            ...
            local sTitle;
            if (type(build.Title) == "function") then
                sTitle = build.Title(build, unit);
            else
                sTitle = build.Title;
            end
            ...
            if (recommendedBuild == nil and 
              ((type(build.Recommended) == "function" and build.Recommended(build, unit)) or
               (type(build.Recommended) ~= "function" and build.Recommended)))
            then
                ...
                Controls.RecommendedActionButton:SetToolTipCallback(function() SetToolTip(sTitle, sToolTip) end);
                Controls.RecommendedActionLabel:SetText(Locale.ConvertTextKey(sTitle));
            end
        end
    end
    --
...
function UpdateUnitPortrait( unit )
    ...
    -- Religion MOD Begin - by CaptainCWB
    if     unit:GetReligion() > ReligionTypes.RELIGION_PANTHEON then
	pPlayer = Players[Game.GetFounder(unit:GetReligion(), -1)];
    elseif unit:GetOriginalOwner() >= 0 and unit:GetOriginalOwner() < GameDefines.MAX_CIV_PLAYERS and unit:GetOriginalOwner() ~= unit:GetOwner() then
	pPlayer = Players[unit:GetOriginalOwner()];
    end
    -- Religion MOD End
    if (pPlayer ~= nil) then
...
function UpdateCityPortrait(city)
	...
	Controls.UnitPortraitButton:SetToolTipString(nil);
...
function UpdateUnitPromotions(unit)
    ...
    for unitPromotion in GameInfo.UnitPromotions() do
        ...
        if (unitPromotion.ShowInUnitPanel == 1 and unit:IsHasPromotion(unitPromotionID) and not unit:IsTrade()) then
...
-- Condensed - MOD
function UpdateUnitPromotionsCondensed(unit)
    g_EarnedPromotionIM:ResetInstances();
...
function UpdateUnitStats(unit)
    ...
    elseif unit:GetDomainType() == DomainTypes.DOMAIN_AIR then
		...
		local ignore_terrain = (unit:IsHasPromotion(GameInfoTypes.PROMOTION_IGNORE_TERRAIN_COST) or unit:IsHasPromotion(GameInfoTypes.PROMOTION_STEAM_POWERED)) and "*" or "";
   ...
   -- Religious units
    ...
    elseif (unit:GetTourismBlastStrength() > 0) then
        ...
        Controls.UnitStatStrength:SetToolTipString(strengthTT);
        Controls.UnitStatNameStrength:SetToolTipString(strengthTT);    
    elseif (unit:IsTrade() and Game.GetTradeRoute) then
	    -- Handled above
    else
        Controls.UnitStrengthBox:SetHide(true);
    end        
...
function OnInfoPaneDirty()
    ...
    if (unit ~= nil) then
        ...
        UpdateUnitPromotionsCondensed(unit);
...
function OnUnitActionClicked( action )
	if bOkayToProcess then
		...
		local pBuild = GameInfo.Builds[GameInfoActions[action].MissionData];
		local pImpPB = GameInfo.Improvements["IMPROVEMENT_PONTOON_BRIDGE_MOD"];
		-- Remove route also removing the bridge
		if ( pBuild ~= nil and pBuild.Type == "BUILD_REMOVE_ROUTE" and pPlot and pPlot:IsWater() and pImpPB ~= nil and pPlot:GetImprovementType() == pImpPB.ID) then
			pPlot:SetImprovementType(-1);
			pPlot:SetRouteType(-1);
			for i = 0, pPlot:GetNumUnits() - 1, 1 do
				local onBridgeUnit = pPlot:GetUnit( i );
				if onBridgeUnit and onBridgeUnit:GetDomainType() == DomainTypes.DOMAIN_LAND and not onBridgeUnit:IsEmbarked() then
					onBridgeUnit:SetEmbarked(true);
					onBridgeUnit:SetMoves(0);
				end
			end
			return;
		-- Fix workers Alert in water - change Alert order to Automate Build -- NOT ACTIVE NOW !!!
		elseif GameInfoActions[action].Type == "MISSION_ALERT" and not pUnit:IsCombatUnit() and pUnit:CanDoCommand(CommandTypes.COMMAND_AUTOMATE,0) then
			pUnit:DoCommand(CommandTypes.COMMAND_AUTOMATE,0);
			return;
		-- MOD End
	
		elseif (GameInfoActions[action].SubType == ActionSubTypes.ACTIONSUBTYPE_PROMOTION) then
...
function TipHandler( control )
    ...
    if (bDisabled) then
		...
		if (bBuild) then
			...
			elseif (pImprovement and not pImprovement.OutsideBorders and not pImprovement.IgnoreOwnership) then
				if (pPlot:GetTeam() ~= unit:GetTeam()) then
