...
MapModData.BoazovazziCode = MapModData.BoazovazziCode or {}
tableRoot = MapModData.BoazovazziCode
tableName = "BoazovazziCode"

include("Sami_TSLSerializerV3.lua");
TableLoad(tableRoot, tableName)

MY_MOD_NAME = "BoazovazziCode"; MY_MOD_SAVE = MapModData.BoazovazziCode;
...
function BoazovazziCode(iPlayer, iX, iY, iImprovement)
	...
	local sPlot = CompilePlotID(pPlot)
	MY_MOD_SAVE[iPlayer][sPlot] = {}
	MY_MOD_SAVE[iPlayer][sPlot].iTurns =		iGolbalTurns
	MY_MOD_SAVE[iPlayer][sPlot].iResource = 	iRes
	
	local iDeltaF = math.ceil(tDelta[GetRandom(1,#tDelta)])
	local iDeltaP = math.ceil(tDelta[GetRandom(1,#tDelta)])
	
	pNearestCity:ChangeFood(iDeltaF)
...
function RepopulateCode(iPlayer)
	for sPlot, tTable in pairs(MY_MOD_SAVE[iPlayer]) do
		if tTable.iTurns - 1 <= 0 then
			
			local pPlot = DecompilePlotID(sPlot)
			local iOwner = pPlot:GetOwner()		
			pPlot:SetOwner(-1)	
