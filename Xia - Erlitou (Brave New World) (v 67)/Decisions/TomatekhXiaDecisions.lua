	...
	if (pPlayer:GetCivilizationType() == GameInfoTypes.CIVILIZATION_ERLITOU_MOD) then
		if load(pPlayer, "Decisions_XiaFloods") == true then
			if pPlot:IsRiver() then
				...
				if (EraCount < 3) then
					RiverFoodBase = RiverFoodBase;
				elseif (EraCount >= 3) and (EraCount < 6) then
					RiverFoodBase = (RiverFoodBase * 1.5);
				elseif (EraCount >= 6) and (EraCount < 9) then
					RiverFoodBase = (RiverFoodBase * 2);
				elseif (EraCount >= 9) then
					RiverFoodBase = (RiverFoodBase * 2.5);
				end

				local RiverFoodShow = math.ceil(RiverFoodBase);
...
local Decisions_XiaBronzes = {}
	...
	function(pPlayer)		
		...
		if bFoundryCount < 3 then return true, false end
