-- StartOnSnow
-- Author: Neirai and LastSword
-- DateCreated: 8/18/2014 8:54:08 AM
--------------------------------------------------------------
function MoveYupiksToPlot(pPlayer, iPlot)
...
local function InvalidYupikStart(pPlot, pPlayer)
...
local function AuditSnowArea(pPlot, pPlayer)
	...
	if not InvalidYupikStart(pPlot, pPlayer) then
...
local function PlaceYupikBonusResource(plot)
	local n = #ResB
	local k = math.random(n)
...
local function PlaceYupikStratResource(plot)
	local n = #ResS
	local k = math.random(n)
	if plot:GetPlotType() == PlotTypes.PLOT_OCEAN then
...
local function PlaceYupikLuxResource(plot)
	if plot:GetPlotType() == PlotTypes.PLOT_OCEAN then
		local n = #SeaResL
		local k = math.random(n)
		if SeaResL[k] then
			...
			if SpecificPlot then
				if SpecificPlot:GetPlotType() ~= PlotTypes.PLOT_MOUNTAIN and SpecificPlot:GetFeatureType() ~= FeatureTypes.FEATURE_ICE and SpecificPlot ~= pPlot then
					...
					elseif ResSet[TileNum] == "Bonus" then
						PlaceYupikResource(SpecificPlot, "Bonus")
						TileNum = TileNum - 1
					elseif ResSet[TileNum] == "Strategic" then
						PlaceYupikResource(SpecificPlot, "Strategic")
						TileNum = TileNum - 1
					elseif ResSet[TileNum] == "Luxury" then
						PlaceYupikResource(SpecificPlot, "Luxury")
						TileNum = TileNum - 1
					end
				else
					if ResSet[TileNum] == "Blank" then
...
function FindNewYupikStartPlot(Bonus, Strategic, Luxury, pPlayer)
	...
	for plotLoop = Map.GetNumPlots() - 1, 0, -1 do
		...
		if pPlot:GetTerrainType() == TerrainTypes.TERRAIN_SNOW or pPlot:GetTerrainType() == TerrainTypes.TERRAIN_TUNDRA then
			if AuditSnowArea(pPlot, pPlayer) then
				...
				MoveYupiksToPlot(pPlayer, pPlot)
				return true
			end
		end
	end
...
local function CreateYupikStartingLocation(pPlot, pPlayer)
	...
	if FindNewYupikStartPlot(Bonus, Strategic, Luxury, pPlayer) then
		range = range + 1
		for dx = -range, range do
			for dy = -range, range do
...
function FindTheYupik()
	print("Looking For Yupik!")
	...
	if Game:GetGameTurn() <= Game.GetStartTurn() then
		for i, possibleYupik in pairs(Players) do
			if possibleYupik:GetCivilizationType() == GameInfoTypes.CIVILIZATION_YUPIK then
				if possibleYupik:GetCapitalCity() == nil then
					print("Yupik are playing this game!")
					for plotLoop = 0, Map.GetNumPlots() - 1, 1 do
						local pPlot = Map.GetPlotByIndex(plotLoop)
						local pUnit = pPlot:GetUnit()
						if pUnit ~= nil then
							if (pUnit:GetUnitType() == GameInfoTypes.UNIT_SETTLER) and (pUnit:GetCivilizationType() == GameInfoTypes.CIVILIZATION_YUPIK) then
								if pPlot:GetTerrainType() ~= TerrainTypes.TERRAIN_SNOW then
									print("Yupik Found!")
									CreateYupikStartingLocation(pPlot, possibleYupik)
									break
								end
							end
						end
